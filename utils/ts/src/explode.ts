...
export function explode(text: string): particle[] {
  // Assume text is LF
  ...
  if (i < 0) throw new Error("no_pragma")
  i = a.indexOf("\n", i)
  if (i < 0) i = a.length
...
function blank_comments(text: string, with_symbol: string = "@"): string {
  let a = text
...
function replace_at(dest: string, i: number, val: string) {
